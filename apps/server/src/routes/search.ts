...
function calculateRelevance(query: string, file: any): number {
  ...
  if (
    file.aiTags &&
    file.aiTags.some((tag: string) => tag.toLowerCase().includes(queryLower))
  ) {
    score += 30;
  }
  ...
  if (
    file.aiKeywords &&
    file.aiKeywords.some((keyword: string) =>
