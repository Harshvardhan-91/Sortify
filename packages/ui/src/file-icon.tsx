...
export const FileIcon: React.FC<FileIconProps> = ({ 
  ...
  const getFileTypeInfo = (mimeType: string, fileName: string) => {
    ...
    if (mimeType.includes('pdf') || extension === 'pdf') {
      return {
        icon: (
          <div className={`${getIconSize()} flex items-center justify-center bg-red-600 text-white text-xs font-bold rounded`}>
            PDF
          </div>
        ),
        bgColor: 'bg-red-100',
    ...
    if (mimeType.includes('document') || mimeType.includes('word') || ['doc', 'docx'].includes(extension)) {
      return {
        icon: (
          <div className={`${getIconSize()} flex items-center justify-center bg-blue-600 text-white text-xs font-bold rounded`}>
            DOC
          </div>
        ),
        bgColor: 'bg-blue-100',
    ...
    if (mimeType.includes('spreadsheet') || mimeType.includes('excel') || ['xls', 'xlsx'].includes(extension)) {
      return {
        icon: (
          <div className={`${getIconSize()} flex items-center justify-center bg-green-600 text-white text-xs font-bold rounded`}>
            XLS
          </div>
        ),
        bgColor: 'bg-green-100',
    ...
    if (mimeType.includes('presentation') || mimeType.includes('powerpoint') || ['ppt', 'pptx'].includes(extension)) {
      return {
        icon: (
          <div className={`${getIconSize()} flex items-center justify-center bg-orange-600 text-white text-xs font-bold rounded`}>
            PPT
          </div>
        ),
    ...
    // Code files
    if (['js', 'ts', 'jsx', 'tsx', 'py', 'java', 'cpp', 'c', 'html', 'css', 'php', 'rb', 'go'].includes(extension)) {
      return {
